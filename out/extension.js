...
const sponsorClient_1 = require("./sponsorClient");
const earningsStore_1 = require("./earningsStore");
const authStore_1 = require("./authStore");
const adRotator_1 = require("./adRotator");
let adBar;
let sessionBar;
let readyBar;
let sponsorClient;
...
function shouldTrackCommand(cmd) {
    return detectTaskType(cmd) !== undefined;
}
function activate(context) {
    const config = vscode.workspace.getConfiguration("kickbackStatus");
    authStore = new authStore_1.AuthStore(context);
    earningsStore = new earningsStore_1.EarningsStore(context);
    sponsorClient = new sponsorClient_1.SponsorClient(config.get("backendUrl", "http://localhost:3000"), earningsStore.getUserId(), () => authStore.getToken());
    // ── Status bar items ──────────────────────────────────────────────────────
    ...
    readyBar.show();
    // ── Commands ──────────────────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.handleClick", async () => {
        const line = activeRotator?.getCurrentLine();
        if (line) {
            await sponsorClient.recordClick(line.id);
            if (line.url)
                vscode.env.openExternal(vscode.Uri.parse(line.url));
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.showEarnings", () => {
        ...
        if (serverPaise !== undefined) {
            ...
            vscode.window.showInformationMessage(`Kickback Status — Server-verified: ₹${serverRupees} (synced ${minsAgo}m ago). ` +
                `Local tally: ₹${localRupees.toFixed(2)} across ${impressions} impressions.`);
        }
        else {
            vscode.window.showInformationMessage(`Kickback Status — Lifetime earnings: ₹${localRupees.toFixed(2)} across ${impressions} impressions.`);
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.syncEarnings", async () => {
        ...
        vscode.window.showInformationMessage(`Kickback Status — Server-verified: ₹${rupees} across ${result.impressionCount} impressions.`);
    }));
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.openSettings", () => {
        vscode.commands.executeCommand("workbench.action.openSettings", "kickbackStatus");
    }));
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.testAd", () => {
        onLongRunningStart(context, "npm");
        setTimeout(() => onLongRunningEnd(), 10000);
    }));
    // ── Activate with invite code ─────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.activate", async () => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(msg === "invalid_or_used_code"
    ...
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.login", async () => {
        const code = await vscode.window.showInputBox({
        ...
        try {
            ...
            await authStore.setTokens(result.accessToken, result.refreshToken, result.userId);
            vscode.window.showInformationMessage("Kickback Status: Signed in successfully! You will resume earning on your next build.");
        }
        catch (err) {
            const msg = err?.message || "unknown error";
            vscode.window.showErrorMessage(msg === "invalid_code"
    ...
    // ── Set UPI ID ────────────────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.setUpiId", async () => {
    ...
    // ── Request withdrawal ────────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.withdraw", async () => {
        ...
        if (result.error === "upi_not_set") {
            const action = await vscode.window.showWarningMessage("Set a UPI ID first to withdraw earnings.", "Set UPI ID");
        ...
        if (result.ok) {
            ...
            vscode.window.showInformationMessage(`Withdrawal of ₹${rupees} requested! Processed within 7 days via UPI.`);
        }
    }));
    // ── Team Pool ─────────────────────────────────────────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.createTeam", async () => {
        ...
        if (result.ok) {
            vscode.window.showInformationMessage(`Team "${name.trim()}" created! Share this code with your team: ${result.code}`);
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.joinTeam", async () => {
        const code = await vscode.window.showInputBox({
    ...
    context.subscriptions.push(vscode.commands.registerCommand("kickbackStatus.showTeam", async () => {
        ...
        if (info === null) {
            const action = await vscode.window.showInformationMessage("You are not in a team pool.", "Create Team", "Join Team");
            if (action === "Create Team")
                vscode.commands.executeCommand("kickbackStatus.createTeam");
            if (action === "Join Team")
                vscode.commands.executeCommand("kickbackStatus.joinTeam");
            return;
        }
        ...
        const top = info.leaderboard.slice(0, 3)
            ...
            .join(" | ");
        vscode.window.showInformationMessage(`Team "${info.team.name}" — ${info.memberCount} members — Total earned: ₹${total} | Top: ${top}`);
    }));
    // ── VS Code Tasks API ─────────────────────────────────────────────────────
    context.subscriptions.push(vscode.tasks.onDidStartTask((e) => {
        ...
        const taskCmd = taskDef?.command;
        const taskType = taskCmd ? detectTaskType(taskCmd) : undefined;
        onLongRunningStart(context, taskType);
    }));
    context.subscriptions.push(vscode.tasks.onDidEndTask(() => onLongRunningEnd()));
    // ── Terminal Shell Integration (VS Code 1.93+) ────────────────────────────
    const activeExecutions = new Map(); // execution → taskType
    ...
    if (authStore.isAccessTokenExpired()) {
        authStore.getRefreshToken().then(async (rt) => {
            ...
            if (result?.accessToken) {
                ...
                const action = await vscode.window.showWarningMessage("Kickback Status: Session expired. Re-enter your invite code to continue earning.", "Re-activate");
    ...
    sessionBar.hide();
    return {
        beginWait: (taskType) => onLongRunningStart(context, taskType),
        endWait: () => onLongRunningEnd(),
    };
}
function onLongRunningStart(context, taskType) {
    const config = vscode.workspace.getConfiguration("kickbackStatus");
    ...
    if (activeTaskCount === 1) {
        ...
        activeRotator = new adRotator_1.AdRotator(adBar, sessionBar, sponsorClient, earningsStore, config, taskType);
        activeRotator.start();
    }
}
